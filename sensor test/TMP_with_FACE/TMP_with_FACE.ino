...
void showTemp(float temperature) {
  ...
  if (face == 0){
    // 顔の輪郭
    M5.Lcd.drawCircle(centerX, centerY, 50, WHITE);

    // 目
    M5.Lcd.fillCircle(centerX - 20, centerY - 15, 5, WHITE);
    M5.Lcd.fillCircle(centerX + 20, centerY - 15, 5, WHITE);

    // にっこり口
    for (int i = -20; i <= 20; i++) {
      int y = (int)(-0.04 * i * i);  // 放物線（口のカーブ）
    ...
    // 目
    M5.Lcd.fillRect(centerX - 25, centerY - 15, 10, 3, WHITE);
    M5.Lcd.fillRect(centerX + 15, centerY - 15, 10, 3, WHITE);
