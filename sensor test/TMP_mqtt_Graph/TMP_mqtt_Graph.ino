...
const char* client_id = "device-M5-graphtest";
...
void loop() {
  ...
  float tempC = sensors.getTempCByIndex(0); // センサの温度を取得
  ...
  if (tempC > 0) {
    if (dataCount < MAX_DATA_POINTS) {
      tempHistory[dataCount] = tempC;
      dataCount++;
    } else {
      for (int i = 1; i < MAX_DATA_POINTS; i++) {
        tempHistory[i - 1] = tempHistory[i];
      }
      tempHistory[MAX_DATA_POINTS - 1] = tempC;
    }
