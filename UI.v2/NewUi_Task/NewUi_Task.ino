...
void setup() {
  ...
  baseline = idleCounter;
  Serial.printf("Baseline idle count: %lu\n", baseline);
...
void loop() {
  ...
  if (millis() - lastMillis >= 1000) {
    float usage = 100.0 * (1.0 - ((float)idleCounter / baseline));
    Serial.printf("CPU usage (approx): %.2f %%\n", usage);
